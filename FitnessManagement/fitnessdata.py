...
class dataGeneration:
    ...
    def loyalty_pts(self, count, streaks_min, streaks_max, Level_min, Level_max, loyalty_min, loyalty_max):
        ...
        result = []
        for i in range(count):
            result.append([r(streaks_min, streaks_max), r(Level_min, Level_max),
                           "LYL" + str(r(loyalty_min, loyalty_max))])
        return result
    ...
    def user_info(self, count, height_min, height_max, weight_min, weight_max, Fname_min, Fname_max, Mname_min,
        ...
        result = []
        for i in range(count):
            result.append(["USR" + str(i), r(gender_min, gender_max), r(height_min, height_max),
                           r(weight_min, weight_max), "Fname" + str(r(Fname_min, Fname_max)),
                           "Mname" + str(r(Mname_min, Mname_max)), "Lname" + str(r(Lname_min, Lname_max))])
        return result
    ...
    def membership_type(self, count, period_min, period_max, amount_min, amount_max, user_id_min, user_id_max,
        ...
        for i in range(count):
            result.append(["MEM" + str(i), r(period_min, period_max), r(amount_min, amount_max),
                           "USR" + str(r(user_id_min, user_id_max)), "TRAINER" + str(r(trainer_id_min, trainer_id_max))])
        return result
...
if __name__ == '__main__':
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerows(dataGen.diet_plan(count=100, ex_routine_min=20, ex_routine_max=60, cal_min=100, cal_max=350,
                                             recipe_min=5, recipe_max=25))

    filename = "exercise_routine.csv"
    with open(filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(dataGen.l2)
        csvwriter.writerows(dataGen.exercise_routine(count=200, duration_min=30, duration_max=120, cal_min=50,
                                                      cal_max=400, ex_id_min=0, ex_id_max=120))
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerows(dataGen.exercise_list(count=150, description_min=20, description_max=300, type_min=5,
                                                  type_max=300))
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerows(dataGen.loyalty_pts(count=50, streaks_min=0, streaks_max=20, Level_min=1, Level_max=5,
                                                loyalty_min=30, loyalty_max=300))

    filename = "user_info.csv"
    with open(filename, 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(dataGen.l8)
        csvwriter.writerows(dataGen.user_info(count=90, height_min=150, height_max=195, weight_min=45, weight_max=95,
                                               Fname_min=1, Fname_max=200, Mname_min=5, Mname_max=250, Lname_min=5,
                                               Lname_max=250))
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerows(dataGen.membership_type(count=5, period_min=2, period_max=12, amount_min=1000,
                                                     amount_max=10000, user_id_min=1, user_id_max=75,
                                                     trainer_id_min=1, trainer_id_max=20))
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerows(dataGen.trainer(count=30, fee_min=1000, fee_max=10000, name_min=1, name_max=50,
                                             ph_min=1000000000, ph_max=9999999999))
